...
const uploadAudioFile = async (req, res) => {
  ...
  const bucket = storage_client.bucket(userID);
  ...
  busboy.on("file", (fieldname, file, { filename, encoding, mimeType }) => {
    ...
    const newID = uuid.v4();
    const newFile = bucket.file(newID);
...
const downloadAudioFile = async (req, res) => {
  ...
  try {
    const file = storage_client.bucket(req.userID).file(audioFileID);
...
const deleteAudioFileFromStorage = async (userID, filename) => {
  const audioFile = storage_client.bucket(userID).file(filename);
